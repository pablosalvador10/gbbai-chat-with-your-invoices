...
class CosmosDBIndexer:
    def __init__(self, 
        ...
        """
        try:
            self.client = CosmosClient(endpoint_url or os.getenv('AZURE_COSMOSDB_ENDPOINT'), credential=credential_id or os.getenv('AZURE_COSMOSDB_KEY'))
    ...
    def create_container(self, container_name: str, 
                         ...
                         analytical_storage_ttl: Optional[int] = None) -> None:
        """
        ...
        logger.info(f"Container '{container_name}' in database '{self.database.id}' created successfully.")

    def index_data(self, data_list: List[Dict[str, Any]], 
                   id_key: str = 'InvoiceId') -> List[Optional[Dict[str, Any]]]:
        """
        ...
        for i, data in enumerate(data_list):
            try:
                ...
                if id_key in processed_data and processed_data[id_key] not in [None, 'null']:
                    logger.info(f"Duplicating key '{id_key}' as 'id'")
                    processed_data['id'] = processed_data[id_key]
                else:
                    logger.warning(f"Data item {i+1} has a null {id_key}. Skipping this item.")
                    continue

                logger.info("Upserting data item into Cosmos DB")
                response = self.container.upsert_item(processed_data)
                logger.info(f"Data indexed successfully with id: {processed_data['id']}")
                responses.append(response)
            except exceptions.CosmosHttpResponseError as e:
                logger.error(f"Failed to index data item {i+1}: {e}")
                responses.append(None)
            except Exception as ex:
                logger.error(f"An unexpected error occurred while processing data item {i+1}: {ex}")
                responses.append(None)
    ...
    def execute_query(self, query: str):
        ...
        try:
            ...
            if items:
                logger.info(f"Query executed successfully. Retrieved {len(items)} items.")
                return items
            else:
